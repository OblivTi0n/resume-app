...
const ResumeContent = forwardRef<HTMLDivElement, ResumeContentProps>(
  ({ data }, ref) => {
    ...
    // -------------------------------------------------------------------
    // Pagination: Move each content block into pages.
    // -------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------
    // Build the resume content as separate blocks.
    // -------------------------------------------------------------------
    const buildResumeContent = (data: any) => {
      ...
      data.work_experience.forEach((exp: any) => {
        ...
        exp.responsibilities.forEach((resp: any) => {
          ...
          // Use resp.text if it exists, otherwise fallback to resp
          const respText =
...
export default function ResumeView({ data }: ResumeViewProps) {
  ...
  // Set up react-to-print using the contentRef option.
  const handleDownload = useReactToPrint({
    content: () => resumeContentRef.current,
    documentTitle: "Resume - Your Name",
  });
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (container && resumeEl) {
        ...
        // Set transform origin so scaling occurs from the center of the element.
        resumeEl.style.transformOrigin = "center";
  ...
  return (
    <div>
      ...
      {/* The container that controls the available width */}
